package org.firstinspires.ftc.teamcode.util;

public class TeamConstants {

    // =============================
    // Intake Constants
    // =============================
    public static final double INTAKE_INTAKE_POWER = 1;


    // =============================
    // Transfer / Outtake Constants
    // =============================
    public static final double TRANSFER_INTAKE_POWER = -0.3;//what runs in intake
    public static final double TRANSFER_IN_POWER = 1;//what runs otherwise

    public static final double TRANSFER_CLOSED = 0;
    public static final double TRANSFER_REVERSED = -1;

    public static final double outtake_Stop = 0;
    public static final double SLIGHT_REVERSE_OUTTAKE = -0.8;


    // =============================
    // Linkage Servo Positions
    // =============================
    public static final double LINKAGE_REST = 0.75;
    public static final double LINKAGE_SHOOT = 0.75;
    public static final double LINKAGE_SHOOT_FAR = 1;



    // =============================
    // Shooter RPM Targets
    // =============================
    public static final double SHOOTER_MID_RPM = 3700;
    public static final double SHOOTER_FAR_RPM = 4200;

    public static final double SHOOTER_FARAUTO_RPM = 2950;

    public static final double Shooter_BottomThreshold = 2300;

    public static final double SHOOTER_CLOSED = 0;
    public static final double INTAKE_REVERSED = -1 ;
    public static final double INTAKE_STOP =0.0 ;


    // =============================
    // Shooter PID Constants
    // =============================
    //deleted

    // =============================
    // Sensors
    // =============================


}
